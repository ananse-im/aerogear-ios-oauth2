...
open class JsonResponseSerializer : ResponseSerializer {
     ...
     */
    open var validateResponse: (URLResponse?, Data) throws -> Void = { (response: URLResponse?, data: Data) throws -> Void in
        ...
        // validate JSON
        do {
            dataAsJson = try JSONSerialization.jsonObject(with: data, options: JSONSerialization.ReadingOptions(rawValue: 0)) as? [String: AnyObject]
        } catch  _  {
    ...
    */
    open var response: (Data, Int) -> AnyObject? = { (data: Data, Int) -> AnyObject? in
        do {
